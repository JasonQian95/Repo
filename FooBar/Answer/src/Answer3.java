 ...
 * converters - which returns a list of integers p where each element in p is
 * the label of the converter that sits on top of the respective converter in q,
 * or -1 if there is no such converter. For example, answer(3, [1, 4, 7]) would
 * return the converters above the converters at indexes 1, 4, and 7 in a
...
public class Answer3 {
	public static int[] answer(int h, int[] q) {
		...
		for (int i : q) {
			int currH = h - 1;
			int curr = (int) Math.pow(2, h) - 1;
			int prev = -1;
			while (curr != i) {
				if (i <= curr - (int) Math.pow(2, currH)) {
					prev = curr;
					curr = curr - (int) Math.pow(2, currH);
				} else {
					prev = curr;
					curr = curr - 1;
				}
	...
	public static void main(String[] args) {
		int[] in1 = { 7, 3, 5, 1 };
		int[] out1 = { -1, 7, 6, 3 };
		System.out.println(Arrays.toString(answer(3, in1)));
		System.out.println(Arrays.equals(answer(3, in1), out1));

		int[] in2 = { 19, 14, 28 };
		int[] out2 = { 21, 15, 29 };
		System.out.println(Arrays.toString(answer(5, in2)));
		System.out.println(Arrays.equals(answer(5, in2), out2));
	}
}