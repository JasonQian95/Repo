...
typedef struct {
	...
	// tells the compiler that the function should not expect to find any of its arguments in registers (a common optimization), but only on the CPU's stack
...
asmlinkage long interceptor(struct pt_regs reg) {
	// If all pids are monitored, monitored == 2, then see if the pid is stored in the demonitor list
	// If some pids are monitored, monitored == 1, then see if the pid is stored in the monitor list
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// Check if Start and stop monitoring commands trying to montior negative pid.
	if ((cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) && pid < 0) {
		return -EINVAL;
	}
	// Check if start and stop monitoring commands trying to monitor pid belonging to valid process.
	if ((cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) && pid != 0) {
		if (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL){
			return -EINVAL;
		}
	}
	// Check if Intercept and Release commands are called by root.
	if ((cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE) &&  current_uid() != 0){
		return -EPERM;
	}
	// Check if monitoring commands are called by non-root and for some pids
	if ((cmd == REQUEST_START_MONITORING  || cmd == REQUEST_STOP_MONITORING) && current_uid() != 0 && pid != 0){
		spin_lock(&pidlist_lock);
	...
	// Check if monitoring commands are called by non-root for all pids.
	if((cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) && current_uid() != 0 && pid == 0){
		return -EPERM;
	}
	// Check if command is trying to deintercept a syscall that has not been intercepted.
	spin_lock(&pidlist_lock);
	...
	spin_unlock(&pidlist_lock);
	// Check to see if it is trying to stop monitoring pid that is not being monitored.
	if((cmd == REQUEST_STOP_MONITORING)){
		spin_lock(&pidlist_lock);
		// if all pids are monitored then list is used to track unmonitored pids
		if (table[syscall].monitored == 2) {
			// check if its in the list of unmonitored pids
		...
		// if some pids are monitored then list is used to track monitored pids	
		else {
			// check if its not in the list of monitored pids
	...
	// Cannot stop monitoring a pid if the syscall is not interecepted
	spin_lock(&pidlist_lock);
	...
	spin_unlock(&pidlist_lock);
	// Cannot monitor for a syscall that is not interecepted
	spin_lock(&pidlist_lock);
	if((cmd == REQUEST_START_MONITORING) && table[syscall].intercepted == 0){
		spin_unlock(&pidlist_lock);
		return -EINVAL;
	}
	spin_unlock(&pidlist_lock);
	// Check if command is trying intercept an already intercepted syscall.
	spin_lock(&pidlist_lock);
	...
	if(cmd == REQUEST_START_MONITORING){
		spin_lock(&pidlist_lock);
		// if all pids are monitored and list using to track unmonitored pids
		if (table[syscall].monitored == 2) {
			// check if its in the list of unmonitored pids
			if (check_pid_monitored(syscall, pid) == 0) {
				spin_unlock(&pidlist_lock);
		...
		// if some pids are monitored and list using to track monitored pids
		else {
			// check if its in the list of monitored pids
			if (check_pid_monitored(syscall, pid) == 1) {
				spin_unlock(&pidlist_lock);
					return -EBUSY;
			}
	...
	// Release and put original syscall back
	...
	// Monitor
		...
		if (pid != 0){
			spin_lock(&pidlist_lock);
			// check if there's enough memory to add pid
	...
	// Demonitor
		...
		if (pid != 0) {
			spin_lock(&pidlist_lock);
			// if all pids are already monitored, keep track of the one that isn't being monitored
			if (table[syscall].monitored == 2) {
				// track it in the list
				add_pid_sysc(pid, syscall);
				spin_unlock(&pidlist_lock);
			}
			// if not all pids are monitored, monitored != 2, delete it from the monitored list
			else {
...
static int init_function(void) {
	...
	// Hijack the exit_group system call (__NR_exit_group) and save the original in orig_exit_syscall
...
static void exit_function(void)
{
	...
	destroy_list(MY_CUSTOM_SYSCALL);
	sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall;
	destroy_list(__NR_exit_group);
	sys_call_table[__NR_exit_group] = my_exit_group;
